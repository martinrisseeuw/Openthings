<nav class="mainNavigation">
	<ul class="left">
			<li class="left"><a href="index.php" class="logo left"><span class="icon-icon_gift"></span> openthings</a></li>
			<li class="left"><a class="navitem-tutorials" href="tutorials.php">Tutorials</a></li>
			<li class="left"><a class="navitem-designers" href="designers.php">Designers</a></li>
	</ul>
	<a href="#addproject" class="shareDesignBtn right" onClick="AddProject();"><span class="icon-icon_plus"></span> Share</a>
	<a href="#login" class="loginBtn right" onClick="LoginScreen();">Login / register</a>
	<a href="#login" class="loginBtn right" onClick="AccountScreen();">Account</a>
</nav>