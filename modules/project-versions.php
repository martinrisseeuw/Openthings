<section class="projectVersions left">
	<h1 class="gamma left">Project versions</h1> 
	<ul class="projectVersionsList left">
		<li>
			<article>
				<h1 class="epsilon">10 file(s) updated by martinrisseeuw</h1>
				<p>I first started with creating the abstract version of the bourgie lamp with Rhinoceros (mac). I first drew the basic shape in Illustrator(image.1) after that I imported in Rhinoceros. I used the revolve command to create half of the lamps shape.</p> 
				<button><span class="icon-save"></span> Download this version</button> <button><span class="icon-eye"></span> View tutorial</button>
			</article>
		</li>
		<li>
			<article>
				<h1 class="epsilon">10 file(s) updated by martinrisseeuw</h1>
				<p>I first started with creating the abstract version of the bourgie lamp with Rhinoceros (mac). I first drew the basic shape in Illustrator(image.1) after that I imported in Rhinoceros. I used the revolve command to create half of the lamps shape.</p> 

				<p>After creating the model, I created two boxes and aligned them so I would have the pocket for the mold. One big box and one smaller box, so the model would have thickness at the sides. I created different holes for the connection width 8mm depth 5mm.</p>
				<button><span class="icon-save"></span> Download this version</button> <button><span class="icon-plus"></span> Add tutorial</button>
			</article>
		</li>
		<li>
			<article>
				<h1 class="epsilon">10 file(s) updated by martinrisseeuw</h1>
				<p>I first started with creating the abstract version of the bourgie lamp with Rhinoceros (mac). I first drew the basic shape in Illustrator(image.1) after that I imported in Rhinoceros. I used the revolve command to create half of the lamps shape.</p> 

				<p>After creating the model, I created two boxes and aligned them so I would have the pocket for the mold. One big box and one smaller box, so the model would have thickness at the sides. I created different holes for the connection width 8mm depth 5mm.</p>
				<button><span class="icon-save"></span> Download this version</button> <button><span class="icon-plus"></span> Add tutorial</button>
			</article>
		</li>
	</ul>
</section>
<section class="projectUpdate left">
	<form class="left" >
		<h1 class="beta">Upload your project version</h1>
		<p>Did you make any changes to this project? Here can you upload the changed files and explain what you did.</p>
		<input type="file" name="fileToUpload" id="fileToUpload" multiple>
		<textarea placeholder="Version description" rows="6"></textarea>
		<button class="left" ><span class="icon-save"></span> Submit</button>
	</form>
</section>